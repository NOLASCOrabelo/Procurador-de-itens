# Descobridor de AnyDesk na Rede Local
# Versão Corrigida
...
if ($localID) {
    Write-Host "✓ AnyDesk ID Local: $localID" -ForegroundColor Green
} else {
...
for ($i = $startIP; $i -le $endIP; $i++) {
    ...
    if (Test-AnyDeskRunning -IPAddress $ip) {
        ...
        if ($adID) {
            Write-Host "  AnyDesk ID: $adID" -ForegroundColor White
        } else {
            Write-Host "  AnyDesk ID: Não acessível (requer permissões admin)" -ForegroundColor Yellow
        }
        Write-Host ""
    }
}
